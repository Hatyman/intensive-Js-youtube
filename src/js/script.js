...
function bindSlideToggle(trigger, boxBody, content, openClass) {
    ...
    button.element.addEventListener('click', () => {
        if (!(button.active)) { // Проверка статуса меню
            button.active = !(button.active); // Смена статуса
...
function start() {
    gapi.client.init({
        'apiKey': 'AIzaSyC4AaQ36-hOHcjo--T2qeYiy7oG7LGPXak',
        'discoveryDocs': ["https://www.googleapis.com/discovery/v1/apis/youtube/v3/rest"]
    }).then(function () {
        return gapi.client.youtube.playlistItems.list({
            "part": "snippet,contentDetails",
            "maxResults": '6',
            "playlistId": "PL7-a4pKt1YOln8EqGXnIVtLEMPZRUGHcs"
        });
...
function sliceTitle(textSelector, length) {
    document.querySelectorAll(textSelector).forEach(item => {
        item.textContent.trim();

        if (item.textContent.length < length) {

        } else {
            const str = item.textContent.slice(0, length + 1) + "...";
